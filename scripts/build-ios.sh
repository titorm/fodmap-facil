...
# Builds the iOS app using EAS Build
...
echo ""
print_info "Building iOS app with profile: $BUILD_PROFILE"
echo ""
...
# Check for macOS (required for local iOS builds)
if [ "$BUILD_PROFILE" == "development" ] && [ "$(uname)" != "Darwin" ]; then
    print_warning "Local iOS development builds require macOS"
    print_info "Building on EAS servers instead..."
fi
...
if [ "$BUILD_PROFILE" == "development" ]; then
    print_warning "Building development client for iOS..."
    print_info "Note: iOS development builds require Apple Developer account"
    eas build --platform ios --profile development
elif [ "$BUILD_PROFILE" == "preview" ]; then
    print_info "Building preview IPA for iOS..."
    print_info "Note: Preview builds require Apple Developer account"
    eas build --platform ios --profile preview
elif [ "$BUILD_PROFILE" == "production" ]; then
    print_info "Building production IPA for iOS..."
    print_info "Note: Production builds require Apple Developer account"
